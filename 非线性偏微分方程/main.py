# 物理信息神经网络（PINN） 求解偏微分方程（PDE）

"""
We have been given a PDE: du/dx=2du/dt+u and boundary condition（边界条件）: u(x,0)=6e^(-3x)

Independent variables（自变量）: x,t (input)
Dependent variables（因变量）: u (outputs)
我们需要找出在[0,2]范围内的所有x和[0,1]范围内的所有t的u(x,t)
When we solved this problem analytically, we found the solution: u(x,t) = 6e^(-3x-2t)
当我们用解析法解决这个问题时，我们找到了解决方案：u(x,t) = 6e^(-3x-2t)
Our f is（定义了函数f） f = du/dx - 2du/dt - u
"""
import numpy as np
import torch
